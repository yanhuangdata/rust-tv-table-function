...
impl BiLL {
    ...
    pub fn new(data: Vec<Vec<f64>>, data_len: usize, forecast_len: usize) -> Result<Self, Error> {
        ...
        let mut func = |x: &[f64]| bill.llh(x[0], x[1], x[2], bill.datalength);
    ...
    fn llh(&self, t1: f64, t2: f64, t3: f64, datarange: usize) -> f64 {
        ...
        for i in 1..datarange.min(self.datalength) {
            ...
            if !steady {
                let (detf, steady_flag) =
                    self.update_kalman(&y, &mut a, &mut v, &mut p, &mut k, &mut fi);
                steady = steady_flag;
...
impl MultivarModel for BiLL {
    ...
    fn variance(&self, i: usize) -> f64 {
        if i < self.p.len() { self.p[i][0] } else { 0.0 }
    }
...
impl BiLLmv {
    ...
    pub fn new(
        ...
        let scale = 0.1;
        let mut psi = vec![1.0 / scale, 0.5 / scale, 1.0 / scale];
        let mut func = |x: &[f64]| billmv.llh(x[0], x[1], x[2], billmv.datalength);
        let (_, _) = dfpmin(&mut func, &mut psi, DFP_TOLERANCE);
    ...
    fn next_state(
        ...
        for i in start..end.min(data[0].len()).min(data[1].len()) {
            let y = [
                data[0].get(i).copied().flatten(),
                data[1].get(i).copied().flatten(),
            ];
            let y_opt = [y[0], y[1]];
    ...
    fn llh(&self, t1: f64, t2: f64, t3: f64, datarange: usize) -> f64 {
        ...
        for detf in self.next_state(
            &self.data,
            1,
            datarange.min(self.datalength),
            &mut a,
            &mut v,
            &mut p,
            &mut k,
...
impl BiLLmv2 {
    ...
    pub fn new(
        ...
        let scale = 0.1;
        let mut psi = vec![1.0 / scale, 0.5 / scale, 1.0 / scale];
        let mut func = |x: &[f64]| billmv2.llh(x[0], x[1], x[2], billmv2.datalength);
        let (_, _) = dfpmin(&mut func, &mut psi, DFP_TOLERANCE);
...
impl LLB {
    ...
    pub fn new(data: Vec<Vec<f64>>, data_len: usize, forecast_len: usize) -> Result<Self, Error> {
        ...
        let scale = 0.1;
        let mut psi = vec![1.0 / scale, 0.5 / scale, 1.0 / scale];
        let mut func = |x: &[f64]| llb.llh(x[0], x[1], x[2], data_len);
        let (_, _) = dfpmin(&mut func, &mut psi, DFP_TOLERANCE);
        llb.update_all(&psi);
        Ok(llb)
    }
    ...
    pub fn predict(&mut self, predict_var: usize, start: usize) -> Result<(), Error> {
        if predict_var != 0 && predict_var != 1 {
            bail!(
                "Invalid parameter 'predict_var': {} - predict_var must be 0 or 1",
                predict_var
            );
        }
        let n = self.data_len;
        ...
        for i in start..n {
            ...
            if i < self.data[corvar].len() && !self.data[corvar][i].is_nan() {
                fc[i] = self.a[i - 1][predict_var]
                    + sigma * (self.data[corvar][i] - self.a[i - 1][corvar]);
            } else {
                fc[i] = self.a[i - 1][predict_var];
            }
        }
        ...
        let algo: Box<dyn MultivarModel> = if algo_name.starts_with("LLB") {
            ...
            if algo_name == "LLBmv" {
                ...
                if let Some(corr) = correlate {
                    ...
                    if data.len() < 2 {
                        bail!(
                            "Insufficient data: required at least 2 data points, got {}",
                            data.len()
                        );
                    }
